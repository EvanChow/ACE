...
import com.avalon.dao.redis.RedisDaoSupport;
import com.avalon.model.redis.Redis;

@Repository
public class MyRedisDao extends RedisDaoSupport<String,Redis>{
    ...
    public String update(String key,String value){
        return getSet(key.getBytes(),value.getBytes());
    }
    ...
    public Map<String,String> getMapAll(String key){
        ...
        for(Iterator<byte[]> iter= set.iterator();iter.hasNext();){
            String k =new String(iter.next());
            String v =new String(map.get(k));
            result.put(k, v);
        }
        return result;
    }
    ...
    public List<String> getList(String key,int begin,int end){
        ...
        List<byte[]>  list= lRange(key.getBytes(), begin, end);
        for(byte[] b:list){
  ...
  //***********************************************************************************************************//
     ...
     */
    public Boolean hDel(final byte[] k,final byte[] f){
        Boolean result=redisTemplate.execute(new RedisCallback<Boolean>() {
            @Override
            public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
