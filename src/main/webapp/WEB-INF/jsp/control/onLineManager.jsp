	...
	<body class="no-skin" style="background-color: #FFFFFF;width:98%;">
		...
		<div class="col-xs-12">
			...
			<div class="table-header">
			    <!--  -->
				<div class="btn-group">
						<button type="button" class="btn btn-info btn-sm btn-primary" id="table-add" >
						<i class="ace-icon fa fa-plus bigger-120"></i>
						发送消息</button>
						<button type="button" class="btn btn-danger btn-sm btn-primary" id="table-edit" >
						<i class="fa fa-pencil-square-o  bigger-120"></i>
						强制退出</button>
						<button type="button" class="btn btn-info btn-sm btn-primary" id="table-refresh" >
			...
			jQuery(function($) {
				...
				.DataTable( {
					aoColumns: [
					  ...
					  { "title": "在线用户",
						//"mDataProp" : "valid",
						"mDataProp" : function(source,type,val){
						   var key=source.attributeKeys[1];
						   var name=source.attributes[key].primaryPrincipal;
							return name;
						 },
						 "defaultContent": "未知",
						 "bSortable": false 
					  ...
					  { "title": "开始时间",
						"mDataProp":function(source,type,val){
							if(type === 'display'){
								return FormatDate('YYYY/MM/DD hh:mm:ss',source.startTimestamp);
							}
							return '--/--/--'},
						"defaultContent": "",
						"bSortable": false },
					  ...
					  { "title": "进入时间",
						"mDataProp":function(source,type,val){
							if(type === 'display'){
								return FormatDate('YYYY/MM/DD hh:mm:ss',source.lastAccessTime);
							}
							return '--/--/--'},
						"defaultContent": "",
						"bSortable": false },
					  { "title": "操作",
						"mDataProp":function(source,type,val){
							...
							'<button type="button" class="btn btn-info btn-sm btn-primary mymsg">消息</button>' +
							'<button type="button" class="btn btn-danger btn-sm btn-primary">强制退出</button>' +
							'</div>'},
						"defaultContent": "",
						"bSortable": false }
			        ...
			        sAjaxSource: "${hosts}/sessions/dg.do",
				...
				$('#table-add').on('click',function(){
					...
					if(count>0){
						...
						layer.alert('请选中要发送消息的用户', {icon: 5});
					}
				...
				$('#table-edit').on('click',function(){
					...
					if(count>0){
						...
						layer.alert('请选中要强制退出的用户', {icon: 5});
					}
				...
				$('#table-delete').on('click',function(){
					...
					layer.confirm('是否要删除这'+count+'条数据？', {
					  btn: ['是','否'] //按钮
						...
						$.post("${hosts}/user/del.do",{uid:ids},function(result){
					    if(result.success){
				...
				$('#table-search').on('click',function(){
					layer.open({
					  ...
					  shade: 0,
					  area: ['600px', '400px'],
					  content: '${hosts}/jump.do?jsp=/sys/user/user_search' //iframe的url
					});
